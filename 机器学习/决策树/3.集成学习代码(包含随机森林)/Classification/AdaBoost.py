# -*- coding: utf-8 -*-
# !/usr/bin/python
# AdaBoost（Adaptive Boosting，自适应增强）是一种集成学习算法，主要用于分类问题，也可以用于回归。
# 它通过组合多个弱分类器（weak classifiers）来构建一个强分类器。其核心思想是：每次训练一个弱分类器，
# 并根据其分类错误率动态调整样本权重，使得后续分类器更关注之前被错误分类的样本。
